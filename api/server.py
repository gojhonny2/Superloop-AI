...
from core.workspace_analyzer import WorkspaceAnalyzer, is_windows_reserved_name, safe_relpath
...
class RunRequest(BaseModel):
    ...
    manager_max_tokens: int | None = None
    specialist_max_tokens: list[int] = Field(default_factory=list)
...
# ── Provider config for model scanning ──
...
def _fetch_models_from_endpoint(url: str, api_key: str, provider: str) -> list[str]:
    ...
    for item in data.get("data", []):
        ...
        if provider == "openai":
            ...
            if any(skip in lower for skip in ["dall-e", "tts", "whisper", "embedding", "moderation", "babbage", "davinci"]):
                continue
...
async def scan_models():
    ...
    for provider_key, cfg in PROVIDERS.items():
        ...
        has_key = bool(api_key) and api_key != f"your-{provider_key}-key"
        ...
        if cfg.get("endpoint"):
            models = await asyncio.get_event_loop().run_in_executor(
                None, _fetch_models_from_endpoint, cfg["endpoint"], api_key, provider_key
            )
        elif cfg.get("known_models"):
    ...
    ollama_models = await asyncio.get_event_loop().run_in_executor(None, _fetch_ollama_models)
    if ollama_models:
        results.append({
            "provider": "ollama",
            "label": "Ollama (Local)",
...
@app.post("/api/workspace/deep-analyze")
async def deep_analyze_workspace(request: WorkspaceAnalyzeRequest):
    """Deep analysis — reads every file, every line, returns full project profile with suggested goals."""
    ...
    analysis = await asyncio.get_event_loop().run_in_executor(None, analyzer.analyze)
    return {"workspace": analyzer.to_api_response()}
...
async def create_run(request: RunRequest):
    try:
        manager = RuntimeModelFactory.create_model(
            request.manager_descriptor,
            max_tokens=request.manager_max_tokens,
        )
        specialist_tokens = list(request.specialist_max_tokens)
        specialists = [
            RuntimeModelFactory.create_model(
                item,
                max_tokens=specialist_tokens[i] if i < len(specialist_tokens) else None,
            )
            for i, item in enumerate(request.specialist_descriptors)
        ]
...
def _scan_workspace(workspace_path: str, data_path: str) -> dict[str, Any]:
    ...
    for root, dirs, files in os.walk(workspace_path):
        dirs[:] = [
            ...
            if item not in WORKSPACE_SKIP_DIRS
            and not item.startswith(".")
            and not is_windows_reserved_name(item)
        ]
        ...
        for filename in files:
            ...
            total_files += 1
            path = os.path.join(root, filename)
            relative = safe_relpath(path, workspace_path)
            if not relative:
                continue
